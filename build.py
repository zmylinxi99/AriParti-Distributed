...
PREBUILT_BINARIES_DIR = PROJECT_ROOT / 'linux-pre_built' / 'binaries'
...
def copy_prebuilt_solvers():
    if not PREBUILT_BINARIES_DIR.exists():
        log(f"Prebuilt binaries directory not found: {PREBUILT_BINARIES_DIR}")
        return
    for binary_name in PREBUILT_SOLVERS:
        src_file = PREBUILT_BINARIES_DIR / binary_name
        dest_file = BINARIES_DIR / binary_name
        if src_file.exists():
            try:
