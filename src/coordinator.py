...
def unexpected_state(error_info):
    logging.error(error_info)
    raise RuntimeError(error_info)

def parse_ablation_config(raw_config):
    config = json.loads(raw_config)
    if not isinstance(config, dict):
        raise ValueError('ablation config must be a JSON object')
    config.setdefault('isolated_original_solver', True)
    config.setdefault('pre_partition', True)
    config.setdefault('dynamic_splitting', True)
    config.setdefault('terminate_on_demand', True)
    config.setdefault('keep_temp', False)
    config.setdefault('allow_no_bicp_ablation', False)
    config.setdefault('partitioner_extra_args', [])
    if not isinstance(config['partitioner_extra_args'], list):
        raise ValueError('ablation.partitioner_extra_args must be a list')
    config['partitioner_extra_args'] = [str(arg) for arg in config['partitioner_extra_args']]
    return config
...
class Coordinator:
    def __init__(self):
        ...
        logging.debug(f'ablation: {json.dumps(self.ablation_config, sort_keys=True)}')
        logging.debug(f'bicp-enabled: {self.bicp_enabled}')
        logging.debug(f'clause-reduction-enabled: {self.clause_reduction_enabled}')
        logging.debug(f'temp_folder_path: {self.coord_temp_folder_path}')
        logging.debug(f'coordinator-{self.rank} initialization complete')
    ...
    def init_params(self):
        ...
        coordinator_args.add_argument('--ablation-json', type=str, default='{}',
                                help='ablation switch JSON')
        coordinator_args.add_argument('--bicp-enabled', type=int, default=1,
                                help='enable Boolean and interval constraint propagation')
        coordinator_args.add_argument('--clause-reduction-enabled', type=int, default=1,
                                help='enable theory-level clause reduction')
        ...
        self.ablation_config = parse_ablation_config(cmd_args.ablation_json)
        self.isolated_original_solver: bool = bool(self.ablation_config['isolated_original_solver'])
        self.pre_partition_enabled: bool = bool(self.ablation_config['pre_partition'])
        self.terminate_on_demand: bool = bool(self.ablation_config['terminate_on_demand'])
        self.keep_temp: bool = bool(self.ablation_config['keep_temp'])
        self.partitioner_extra_args: list = self.ablation_config['partitioner_extra_args']
        self.bicp_enabled: bool = bool(cmd_args.bicp_enabled)
        self.clause_reduction_enabled: bool = bool(cmd_args.clause_reduction_enabled)
    ...
    def process_partitioner_msg(self, msg: str):
        ...
        if words[0] in ['sat', 'unsat', 'unknown']:
            ...
            if result == 'sat':
                ...
                unexpected_state(f"Unknown partitioner result: {result}")
        else:
            ...
            if op.is_debug_info():
                ...
                unexpected_state(f"Unknown partitioner message operation: {words[0]}")
    ...
    def check_solving_status(self, node: ParallelNode):
        ...
        if sta.is_error():
            ...
            return False
        if sta.is_solving():
            if (not self.terminate_on_demand) or (not self.need_terminate(node)):
                return True
    ...
    # Schedule waiting tasks in generation order.
    ...
    def run_partitioner(self):
        ...
        cmd =  [self.partitioner_path,
                ...
                f'-bicp:{str(self.bicp_enabled).lower()}',
                f'-clause_reduction:{str(self.clause_reduction_enabled).lower()}'
            ]
        cmd.extend(self.partitioner_extra_args)
    ...
    def pre_partition(self):
        if not self.pre_partition_enabled:
            if self.check_original_task():
                return True
            self.terminate_partitioner()
            logging.debug('pre-partition disabled by ablation config')
            MPI.COMM_WORLD.send(ControlMessage.C2L.pre_partition_done,
                                dest=self.leader_rank, tag=1)
            MPI.COMM_WORLD.send(0, dest=self.leader_rank, tag=2)
            msg_type: ControlMessage.L2C = MPI.COMM_WORLD.recv(source=self.leader_rank, tag=1)
            if msg_type.is_assign_node():
                logging.debug('send root-task to coordinator-0 without pre-partition')
                self.send_root_task_to_coordinator(0)
                self.status = CoordinatorStatus.waiting
                return False
            elif msg_type.is_terminate_coordinator():
                raise TerminateMessage()
            else:
                unexpected_state(f"Unexpected leader response after disabled pre-partition: {msg_type}")
        ...
        if msg_type.is_assign_node():
            if pp_num_nodes > 0:
                for i in range(pp_num_nodes):
                    ...
                    logging.debug(f'split node-{self.split_node.id} to coordinator-{i}')
                    self.send_split_node_to_coordinator(i)
            else:
                logging.debug('split root-task to coordinator-0')
                self.send_root_task_to_coordinator(0)
            return False
        elif msg_type.is_terminate_coordinator():
            raise TerminateMessage()
        else:
            unexpected_state(f"Unexpected leader response after pre-partition: {msg_type}")
    ...
    def process_transfer_message(self):
        target_rank = MPI.COMM_WORLD.recv(source=self.leader_rank, tag=2)
        logging.debug(f'split node-{self.split_node.id} to coordinator-{target_rank}')
    ...
    def receive_message_from_leader(self):
        if MPI.COMM_WORLD.Iprobe(source=self.leader_rank, tag=1):
            ...
            if msg_type.is_request_split():
                ...
                unexpected_state(f"Unexpected leader message: {msg_type}")
    ...
    def isolated_solve(self):
        ...
        if self.isolated_original_solver:
            self.solve_original_task()
        else:
            logging.debug('isolated original solver disabled by ablation config')
        ...
        while True:
            if MPI.COMM_WORLD.Iprobe(source=self.leader_rank, tag=1):
                ...
                self.tree_log_display()
                raise TerminateMessage()
            if self.isolated_original_solver and self.check_original_task():
                self.send_result_to_leader()
                self.original_process = None
                continue
            if self.status.is_solving():
    ...
    def clean_temp_dir(self):
        if self.keep_temp:
            logging.info(f'Coordinator-{self.rank} keep temp dir {self.temp_dir}')
            return
    ...
    def __call__(self):
        try:
            ...
            logging.info(f'Coordinator-{self.rank} terminated by leader')
