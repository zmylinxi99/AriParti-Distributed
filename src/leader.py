...
class Leader:
    ...
    def init_params(self):    
        ...
        self.dynamic_splitting_enabled: bool = bool(self.ablation_config['dynamic_splitting'])
        
        if not os.path.exists(self.input_file_path):
            raise FileNotFoundError(f"Input SMT-LIB file not found: {self.input_file_path}")
    ...
    def check_coordinators(self):
        ...
        while MPI.COMM_WORLD.Iprobe(source=MPI.ANY_SOURCE, tag=1, status=msg_status):
            ...
            if msg_type.is_split_succeed():
                ...
                unexpected_state(f"Unexpected coordinator message from {src}: {msg_type}")
        return False
        ...
        while not os.path.exists(src_done_path):
            if MPI.COMM_WORLD.Iprobe(source=self.isolated_rank, tag=1):
                ...
                if msg_type.is_notify_result():
                    ...
                    unexpected_state(f"Unexpected isolated coordinator message: {msg_type}")
            time.sleep(0.01)
    ...
    def assign_node_to_idle_coordinator(self):
        if not self.dynamic_splitting_enabled:
            return
    ...
    def __call__(self):
        try:
            ...
            if status.is_sat():
                ...
                unexpected_state(f"Unexpected final status: {status}")
            self.tree.log_display()
