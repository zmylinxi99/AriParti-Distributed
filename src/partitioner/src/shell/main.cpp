...
static void parse_cmd_line_args(std::string& input_file, int argc, char ** argv) {
    ...
    while (i < argc) {
        ...
        if (arg[0] == '-' 
            ...
            else if (strcmp(opt_name, "bicp") == 0) {
                if (!opt_arg)
                    error("option argument (-bicp:0/1) is missing.");
                gparams::set("bicp_enabled", opt_arg);
            }
            else if (strcmp(opt_name, "clause_reduction") == 0) {
                if (!opt_arg)
                    error("option argument (-clause_reduction:0/1) is missing.");
                gparams::set("clause_reduction_enabled", opt_arg);
            }
            else {
