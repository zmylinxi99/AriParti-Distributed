...
context_t::context_t(reslimit& lim, config_mpq const & c, params_ref const & p, small_object_allocator * a):
    ...
    m_init(false),
    m_bicp_enabled(true),
    m_clause_reduction_enabled(true),
    m_best_var_info(nm()),
    m_curr_var_info(nm()),
...
void context_t::normalize_bound(var x, const numeral &val, numeral &result, bool lower, bool & open) {
    ...
    if (is_int(x)) {
        ...
        // The mpq configuration is exact. Replacing a large-denominator real
        // bound by an outward 10^-6 approximation can weaken an original unit
        // atom that is subsequently omitted from the generated task. Keep the
        // exact rational bound so emitted bounds preserve the represented task.
        nm().set(result, val);
    }
...
void context_t::propagate(node * n, bound * b) {
    ...
    for (; it != end; ++it) {
        ...
        try {
            if (w.is_clause()) {
                clause * c = w.get_clause();
                if (m_bicp_enabled && may_propagate(b, c, n)) {
                    propagate_clause(c, n);
                }
            }
            else {
...
void context_t::init_partition() {
    ...
    m_bicp_enabled = p.get_bool("bicp_enabled", true);
    m_clause_reduction_enabled = p.get_bool("clause_reduction_enabled", true);
...

    {
        ...
        m_temp_stringstream << "bicp_enabled: " << m_bicp_enabled;
        write_debug_ss_line_to_coordinator();

        m_temp_stringstream << "clause_reduction_enabled: " << m_clause_reduction_enabled;
        write_debug_ss_line_to_coordinator();
    }
}
...
bool context_t::convert_node_to_task(node * n) {
    ...
    if (!m_bicp_enabled) {
        ...
        if (!encode_all_variables) {
            for (unsigned i = 0, sz = m_unit_clauses.size(); i < sz; ++i) {
                ...
                vector<lit> unit_clause;
                unit_clause.push_back(std::move(convert_atom_to_lit(at)));
                task.m_clauses.push_back(std::move(unit_clause));
            }
        }

        for (unsigned x = 0, sz = num_vars(); x < sz; ++x) {
            if (!encode_all_variables && m_defs[x] != nullptr)
                continue;
            if (m_is_bool[x]) {
                if (n->bvalue(x) == bvalue_kind::b_undef)
                    continue;
                task.m_var_bounds.push_back(lit());
                lit & l = task.m_var_bounds.back();
                l.m_x = x;
                l.m_bool = true;
                l.m_open = false;
    ...
    for (unsigned i = 0, isz = m_clauses.size(); i < isz; ++i) {
        ...
        if (m_clause_reduction_enabled) {
            ...
            if (lit_cla.size() == 1) {
                temp_units.push_back(std::move(lit_cla[0]));
            }
            else {
    ...
    if (m_clause_reduction_enabled) {
        ...
        for (unsigned i = 0, sz = temp_clauses.size(); i < sz; ++i) {
            task.m_clauses.push_back(std::move(temp_clauses[i]));
        }
    }
