...
class NodeStatus(Enum):
    ...
    def is_delegated(self):
        return self == NodeStatus.delegated

    def is_locally_closed(self):
        return self.is_unsat() or self.is_delegated()
    
    def is_error(self):
        return self == NodeStatus.error
    
    def is_ended(self):
        return self.is_solved() or self.is_terminated() or self.is_delegated()
...
class ParallelNode(PartitionNode):
    ...
    def can_close_local_region(self):
        if len(self.children) < 2:
            return False
        for child in self.children:
            child: ParallelNode
            if not child.status.is_locally_closed():
                return False
        return True
...
class ParallelTree(PartitionTree):
    ...
    def select_split_node(self):
        ...
        while True:
            ...
            assert((not lc.status.is_locally_closed()) or (not rc.status.is_locally_closed()))
            if lc.status.is_locally_closed():
                current = rc
            elif rc.status.is_locally_closed():
                current = lc
            else:
    ...
    def unsat_push_up(self, node: ParallelNode):
        if node.status.is_unsat():
            return
        if node.can_close_local_region():
    ...
    def unsat_push_down(self, node: ParallelNode):
        if node.status.is_unsat() or node.status.is_delegated():
            return
    ...
    def node_delegated(self, node: ParallelNode):
        """Close a region only in the donor's local ownership tree.

        The distributed leader records the transferred region as a separate open
        child obligation.  Delegation is therefore not an UNSAT proof and must
        not be overwritten by ancestor UNSAT propagation in the donor tree.
        """
        self.update_node_status(node, NodeStatus.delegated, NodeReason.split)
    ...
    def node_solved(self, 
        ...
        if status.is_sat():
            ...
            unexpected_state(f"Unsupported parallel node solve status: {status}")
...
class DistributedTree(PartitionTree):
    ...
    def node_partial_solved(self, 
        ...
        if status.is_sat():
            ...
            unexpected_state(f"Unsupported distributed partial status: {status}")
